function [currentPosition, candidateModel, orientation, dimensions] = CAMShift(currentFrame, targetModel, ...
    previousPosition, calculationBandwidth, maxIterations, stopThreshold, binIdxMapFcnHandle)
...
% interface - Gary R. Bradski
    ...
    calculationRegionMinX = max(1, previousPosition(1) - round(maxRadious * calculationBandwidth));
    calculationRegionMaxX = min(size(currentFrame,2), previousPosition(1) + round(maxRadious * calculationBandwidth));
    calculationRegionMinY = max(1, previousPosition(2) - round(maxRadious * calculationBandwidth));
    calculationRegionMaxY = min(size(currentFrame,1), previousPosition(2) + round(maxRadious * calculationBandwidth));
    
    % Compute bin index map for calculation region
    currentFrameBinIdxMap = binIdxMapFcnHandle(double(currentFrame(calculationRegionMinY:calculationRegionMaxY, ...
    ...
    currentPosition = previousPosition - [calculationRegionMinX, calculationRegionMinY];
    ...
    for i = 1:maxIterations
        ...
        searchWindowMinX = currentPosition(1) - maxRadious;
        searchWindowMaxX = currentPosition(1) + maxRadious;
        searchWindowMinY = currentPosition(2) - maxRadious;
        searchWindowMaxY = currentPosition(2) + maxRadious;
        ...
        newPosition(1) = M10 / M00;
        newPosition(2) = M01 / M00;

        % Shfit new position to global coordinate system
        newPosition(1) = newPosition(1) + searchWindowMinX;
        newPosition(2) = newPosition(2) + searchWindowMinY;
    ...
    currentPosition = currentPosition + [calculationRegionMinX, calculationRegionMinY];
